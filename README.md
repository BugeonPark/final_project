...
**1. N+1 해결**
- MyBatis의 resultMap에서 collection(List)을 조회하게 됩니다.
<resultMap type="Food" id="FoodMap">
		...
		<association column="food_category_idx" property="foodCategory" 
			javaType="FoodCategory" select="com.sds.weatherapp.model.food.FoodCategoryDAO.selectByIdx"/>
		<collection column="food_idx" property="tastes" javaType="java.util.List"
			ofType="FoodTaste" select="com.sds.weatherapp.model.food.FoodTasteDAO.selectByFoodIdx"/>
</resultMap>
- 음식을 조회하려 하는데 collection의 select로 맛을 조회하기 위해 N번 쿼리를 실행하게 됩니다.
- 쿼리를 join을 수행하여 데이터를 한번에 가져오도록 하는 nested result 방식을 통해 해결하였습니다.
...
#### 개요
- 음식과 식당을 추천하는 과정에서 설문조사를 진행한다면 해당 설문조사에 대해 감정분석을 통해 보다 정교한 추천이 가능해집니다.
- 하지만, 설문조사를 강제하게 된다면 사용성을 해치게 되므로 설문조사를 강제하지 않기로 결정했습니다.
- 이로 인해, 관계형 데이터베이스일 경우, 일부 사용자만 설문조사 데이터를 가지게 되어 데이터 관리에 어려움이 생겼습니다.

#### 어려움
**1. 데이터에 대한 관리**
- 회원과 설문조사에 대한 테이블을 inner join을 할 경우 결과값을 누락할 수 있습니다.
- 설문조사에 대한 감정 조사를 진행할 경우, 결과 왜곡이 생길 수 있습니다. NULL에 대한 데이터는 집계에서 제외가 됩니다.
- 정합성 문제가 발생할 수 있습니다. 데이터의 일관성이 떨어지고, NULL에 대한 추가적인 처리가 필요해지며, 결과가 누락되므로 데이터에 대한 분석에 혼란이 발생 할 수 있습니다.
- NULL에 대해 명시적으로 처리를 해야합니다.
- 추천을 하는 로직에서 NULL일 경우를 가정하고 데이터 무결성 검증이나 로직 설계가 복잡해 집니다. 예외 처리, 조인을 진행한 후 얻는 결과에 따라 조건부 로직을 추가해야 합니다.

**2. 스키마 변경시**
- 추천로직이나 감정 분석 모델에 따라 스키마를 변경할 경우 비즈니스 로직을 수정해야 하거나 검증 과정을 꼼꼼이 거쳐야 합니다.
- NULL값을 가지는 데이터와 충돌하지 않도록 하여 무결성에 문제가 없게 해야 합니다.
- NULL값을 가지는 행들에 대해 인덱스를 잘 활용하지 못합니다. 스키마 변경 후 인덱스를 다시 설정하지 않으면 성능 저하가 될 수 있습니다.
- 기존의 NULL을 처리하기 위한 쿼리가 변경 되어야 하며 기존 쿼리들이 비정상적으로 동작할 수 있습니다.
- 기존 데이터가 유효하지 않게 될 수 있습니다.

#### No SQL 도입
- MongoDB를 도입하여 자유로운 스키마를 통해 위의 문제들을 해결하고자 하였습니다.
- 누구는 있고 누구는 없는 데이터에 대해 자유로운 스키마를 이용하여 필드하나 추가하기만 하면 되었습니다. 각각의 사용자별로 다른 필드를 가지고 있을 수 있는 문제를 해결했습니다.
- 복잡한 join이나 쿼리를 작성할 필요가 없어졌습니다.
- JSON 형태로 제공되기 때문에 API 결과값을 가공 없이 그대로 저장하거나 불러와 편리함을 얻었습니다.
...
**1. Vue.js**
- 사용자 별로 로그인 유무, 가입 경로(oAuth와 홈페이지), 주소 상세정보의 유무에 따라 화면이 다르게 보여져야 했습니다.
- 주소 상세정보가 없다면 이를 입력하도록 해야 했고, jwt 방식을 통해 로그인 유무에 따라 로그인 페이지로 이동하게끔 해야 했습니다.
- jQuery ajax 비동기 통신 방법으로 우선 jwt 인증 방식을 거치고, 유효한 사용자라면 vue.js를 통해 동적으로 화면을 렌더링 하였습니다.

**2. thymeleaf
- 초기 메인 페이지, 로그인 페이지등 정적인 화면에 대해 thymeleaf를 통해 서버에서 클라이언트에게 전달하도록 하였습니다.

</br>

## 배포
- 네이버 클라우드 우분투 서버에 배포하였습니다.
- 우분투 소보애 자바, MySQL을 설치하였습니다.
- user를 만들어서 리눅스 로컬 사용자를 FTP 사용자로 등록했습니다.
- ftp 서버를 구축하고 방화벽을 설정하여 리눅스 사용자를 ftp 사용자로 설정후 tomcat을 설치했습니다.
- filezilla를 이용하여 war파일로 빌드한 코드들을 업로드 후, 실행하였습니다.
- 효율적인 CI/CD 파이프라인에 관심이 생겼습니다.
...
**1. MVC 패턴**
- 컨트롤러는 요청만을 처리하도록 하여 비대해지지 않도록 하였습니다.
- 화면과 관련된 코드들은 vue.js, thymeleaf를 통해 자바 코드가 변하여도 변경사항이 없도록 설계하였습니다.
- DAO 단계에서는 단일 쿼리만을 처리하고 서비스 단계에서 호출하여 트랜잭션을 처리하도록 설계하였습니다.

**2. 객체 지향적 설계**
- 인터페이스를 통해 구현체를 구현하였고, 각 클래스는 스프링의 DI를 통해 인터페이스를 참조하도록 하였습니다.
- 각 클래스는 단일 기능에 집중하도록 설계하였습니다.
- 구현체가 변경되더라도 기존 코드들은 DI를 통해 느슨한 연결관계가 형성되어 있어 변경사항이 없습니다.
